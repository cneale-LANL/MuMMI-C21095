...
AminoAcids    = "ALA CYS ASP GLU PHE GLY HIS ILE LYS LEU MET ASN PRO GLN ARG SER THR VAL TRP TYR ACE NH2 CYP CYF CYM NMA".split()
...
def write_gro(filename, title, atoms, box, size):
    ...
    for atom in atoms:
        ...
        # Chris Neale is not sure why the kick was turned off with commented out command below, but I am turning it back on here
        ...
        # as long as the decimals are equally spaced, the gro will accept more precision 
        # this will help to avoid crashes from overlapping atoms, which was not a problem in gmx 5.1.5 but is now a problem in gmx 2019.6
        dev.write("%5d%-5s%5s%5d%10.5f%10.5f%10.5f\n"%(id%1e5,res,nam,idx%1e5,x,y,z))
        idx += 1
